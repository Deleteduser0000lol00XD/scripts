...
local OrionLib = {
	...
	Themes = {
		Default = {
			...
			Stroke = Color3.fromRGB(76, 125, 138),
			Divider = Color3.fromRGB(60, 60, 60),
			Text = Color3.fromRGB(100, 116, 122),
...
local freeMouse = Create("TextButton", {Name = "FreeMouse", Size = UDim2.new(0,0,0,0), BackgroundTransparency = 1, Text = "", Position = UDim2.new(0,0,0,0), Modal = true, Parent = Orion, Visible = false})
...
CreateElement("TextBox", function(Text, Placeholder)
...
function OrionLib:MakeWindow(WindowConfig)
	...
	
	{
		...
		SetProps(MakeElement("AspectRatio"), {
			DominantAxis = 0,
			AspectRatio = 0.986,
			AspectType = 1
		})
	...
	local SearchBox = AddThemeObject(SetChildren(SetProps(MakeElement("Frame"), {
        Name = "SearchBoxFrame",
        ...
        MakeElement("Corner", 0, 13),

        SetProps(Create("UIStroke"), {
            Name = "SearchStroke",
	...
	AddConnection(SearchBox.Input:GetPropertyChangedSignal("Text"), function()
		local Input = SearchBox.Input.Text:lower()

		for _, btn in pairs(TabHolder:GetChildren()) do
			if btn:IsA("TextButton") then
				local btnText = ""

				if btn:FindFirstChild("Title") and btn.Title:IsA("TextLabel") then
					btnText = btn.Title.Text
				else
					btnText = btn.Name
				end

				if Input == "" or btnText:lower():find(Input) then
					btn.Visible = true
				else
					btn.Visible = false
				end
...
local WindowName = AddThemeObject(SetProps(MakeElement("Label", "", 14), {
...
local targetText = WindowConfig.Name
...
local function TypeAnimation()
	while not animationComplete do
		if not isDeleting and not isRestoring then
			if currentCharIndex <= #targetText then
				WindowName.Text = string.sub(targetText, 1, currentCharIndex) .. ""
				currentCharIndex = currentCharIndex + 1
				wait(typingSpeed)
			else
				WindowName.Text = targetText .. ""
				wait(pauseTime)
				isDeleting = true
			end
...
spawn(TypeAnimation)
	...
	AddConnection(CloseBtn.MouseButton1Up, function()
		...
		OrionLib:MakeNotification({
			Name = "UI has been hidden.",
			Content = "Press "  .. WindowConfig.KeyToOpenWindow .. " to reopen the UI. ",
			Time = 2
		})
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				ToggleConfig.Color = ToggleConfig.Color or Color3.fromRGB(120, 120, 120)
			...
			function ElementFunction:AddSlider(SliderConfig)
				...
				SliderConfig.Color = SliderConfig.Color or Color3.fromRGB(120, 120, 120)
			...
			function ElementFunction:AddPlayersDropdown(DropdownConfig)
                ...
                if type(DropdownConfig.Default) == "string" and DropdownConfig.Default ~= "" then
                    Dropdown.Value[DropdownConfig.Default] = true
                ...
                function Dropdown:UpdateVisuals()
                    ...
                    for name, btn in pairs(Dropdown.Buttons) do
                        if Dropdown.Value[name] then
                            local cleanName = name:gsub("%s*%(ID:.*%)", "")
                            table.insert(selectedNames, cleanName)
                    ...
                    if #selectedNames > 0 then
                        ...
                        DropdownFrame.F.Selected.Text = "unknown"
                    end
                end
                ...
                function Dropdown:Set(Value)
                    ...
                    local selectedList = {}
                    for name, _ in pairs(Dropdown.Value) do
			...
			function ElementFunction:AddColorpicker(ColorpickerConfig)
                ...
                local RgbLabel = AddThemeObject(SetProps(MakeElement("Label", "0  0  0", 12), {
                ...
                RgbLabel.Parent = InfoContainer

                local HexLabel = AddThemeObject(SetProps(MakeElement("Label", "#000000", 12), {
                    Size = UDim2.new(1, 0, 0, 24),
                ...
                AddConnection(Color.InputBegan, function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        ...
                        local ColorX = (math.clamp(Input.Position.X - Color.AbsolutePosition.X, 0, Color.AbsoluteSize.X) / Color.AbsoluteSize.X)
                ...
                local Dragging_1, DragInput_1 = false, nil

                AddConnection(Hue.InputBegan, function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        Dragging_1 = true
                        
                        local HueX = (math.clamp(Input.Position.X - Hue.AbsolutePosition.X, 0, Hue.AbsoluteSize.X) / Hue.AbsoluteSize.X)
                        HueSelection.Position = UDim2.new(HueX, 0, 0.5, 0)
                        ColorH = 1 - HueX
                        UpdateColorPicker()
